 ...
 * @author KandL
 */
public class ControladorGrado extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (gra.equalsIgnoreCase("")) {
            String error = "Debe llenar el grado";
            request.getSession().setAttribute("error", error);
            request.getRequestDispatcher("errorAdmi.jsp").forward(request, response);
        } else {
            Grado g = new Grado(gra,año);
            if(g.insert()==false){
            String error = "No se Inserto el Grado";
            request.getSession().setAttribute("error", error);
            request.getRequestDispatcher("errorAdmi.jsp").forward(request, response);   
            }else{
            request.getSession().setAttribute("grado", g);
